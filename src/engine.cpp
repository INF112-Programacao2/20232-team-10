...
void Engine::game_settings(){
    ...
    //Imagem de fundo
    auto picture = tgui::Picture::create("./assets/criacaopersn.png");
...
void Engine::pass_screen(Player *player) {
    ...
    auto picture = tgui::Picture::create("./assets/capypass.png");
...
void Engine::main_menu() {
    ...
    auto picture = tgui::Picture::create("./assets/4pilastrascc.jpg");
    picture->setSize({"100%", "100%"});
    gui.add(picture);
    ...
    if (!horrorgame.openFromFile("./assets/8bit-Syndrome-Horror-audio01-.ogg"))
        return ; // error
    horrorgame.play();
    while (true) {
        sf::Event main_menu;
